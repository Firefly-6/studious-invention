...
// 设计：需求类=对选品者意味着"市场机会/热销信号"；供给类=产能/进口/生产/物流侧。
const KEYWORDS = [
  // 需求（消费、热销、零售、电商平台、短缺）——对卖家=机会信号
  { re: /спрос|дефицит|нехватк|потреблен|потребител|покупат|закупк|заказ|продаж|популярн|востребован|товар|ритейл|розниц|маркетплейс|ozon|wildberries|магазин|онлайн-торговл|электронн.{0,6}торговл/i, tag: '需求↑' },
  // 供给 / 生产 / 进出口
  { re: /поставк|производств|импорт|экспорт|предложен|завод|фабрик|сокращен|снижен|снизил|сократ|перебои/i, tag: '供给↓' },
  // 价格
  { re: /цен[аы]|стоимост|подорож|подешев/i, tag: '价格' },
  // 物流
  { re: /логистик|доставк|транспорт|таможн/i, tag: '物流' },
  // 制裁（背景）
  { re: /санкц/i, tag: '制裁' },
  // 汇率（背景）
